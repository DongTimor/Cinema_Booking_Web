@foreach ($vouchers as $voucher)
    <div class="voucher-card my-2">
        ...
        <button type="button" class="btn use-btn text-uppercase rounded-md border-0 shadow-sm" data-bs-dismiss="modal"
            data-id="{{ $voucher->id }}" data-value="{{ $voucher->value }}"
            data-type="{{ $voucher->type }}">Use</button>
    </div>
@endforeach
